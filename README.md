...
- **CompassChromeApplicationCustomizer** — a site-wide extension that injects the header (with hover mega-menus and an AI assistant entry point) and footer into every page of the site. The assistant chat calls the tenant's existing AskStarBulk backend (`chatResourceUri`/`chatQueryUrl` in `sharepoint/assets/elements.xml`) via SPFx's built-in `AadHttpClientFactory`, authenticated as the signed-in user — no API key involved on our side. This requires the `webApiPermissionRequests` entry in `config/package-solution.json` to be approved once by a tenant admin under SharePoint Admin Center → Advanced → API access after the first deployment. If `chatResourceUri`/`chatQueryUrl` are left blank, `ChatWidget` falls back to `chatApiUrl` (our own `api/` proxy below) and then to a local mock reply pool.
- **api/** — an optional Azure Function backend that proxies chat messages to OpenAI and fleet vessel positions to the company's fleet-tracking provider, keeping both API keys server-side; only used as a fallback when the AskStarBulk backend above isn't configured. Configure `OPENAI_API_KEY`, `FLEET_API_URL`, and `FLEET_API_KEY` in the function app's settings (see `api/local.settings.json.template` for local development).
